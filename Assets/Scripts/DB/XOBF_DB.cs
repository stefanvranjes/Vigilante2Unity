...
public class XOBF_DB : MonoBehaviour
{
    ...
    public void SetAtlas()
    {
        ...
        billboard3.SetFloat("_ColorOnly", 0);
        billboard3.SetFloat("_Unlit", 1);
        billboard3.SetFloat("_OffsetFactor", -2);
        billboard3.SetFloat("_OffsetUnits", -2);
        billboard3.SetFloat("_RenderMode", 2);
        billboard3.SetFloat("_Cul", 1);
        billboard3.SetFloat("_DrawDist", 0);
        additive2 = new Material(Shader.Find("PSXEffects/PS1Additive2"));
        additive2.mainTexture = atlas;
        additive2.SetFloat("_ColorOnly", 0);
        additive2.SetFloat("_Unlit", 1);
        additive2.SetFloat("_OffsetFactor", -2);
        additive2.SetFloat("_OffsetUnits", -2);
        additive2.SetFloat("_RenderMode", 2);
        additive2.SetFloat("_Cul", 2);
        additive2.SetFloat("_DrawDist", 0);
        transparent = new Material(Shader.Find("PSXEffects/PS1Transparent"));
        transparent.mainTexture = atlas;
        transparent.SetFloat("_ColorOnly", 0);
        transparent.SetFloat("_Unlit", 1);
        transparent.SetFloat("_OffsetFactor", -2);
        transparent.SetFloat("_OffsetUnits", -2);
        transparent.SetFloat("_RenderMode", 1);
        transparent.SetFloat("_Cul", 1);
        transparent.SetFloat("_DrawDist", 0);
        ...
        transparent.EnableKeyword("TRANSPARENT");
        ...
        billboard4.SetFloat("_ColorOnly", 0);
        billboard4.SetFloat("_Unlit", 1);
        billboard4.SetFloat("_OffsetFactor", 1);
        billboard4.SetFloat("_OffsetUnits", 1);
        billboard4.SetFloat("_RenderMode", 2);
        billboard4.SetFloat("_Cul", 1);
        billboard4.SetFloat("_DrawDist", 0);
    }
    ...
    public Material[] GetMaterialList(VigMesh mesh, int tmdID)
    {
        ...
        MemoryStream stream = new MemoryStream(mesh.faceStream);
            ...
            if (0 < mesh.faces)
            {
                for (int i = 0; i < mesh.faces; i++)
                {
                    ...
                    switch ((uint)bVar1 >> 2 & 15)
                    {
                        ...
                        case 10:
                            iVar3 = 0;
                            int flareCount = reader.ReadUInt16(10);
        ...
        for (int i = 0; i < materialIDs.Count; i++)
        {
            ...
            {
                mesh.materialIDs.Add(materialIDs[i], (int)renderList[materialIDs[i]] + 1);
            }
        }
    ...
    public VigMesh FUN_1FD18(GameObject param1, uint param2, bool init)
    {
        ...
        List<OcclusionCamera> occlusionList = new List<OcclusionCamera>();
                        ...
                        switch ((uint)bVar1 >> 2 & 15)
                        {
                            ...
                            case 10:
                                ...
                                if (flareCount != 0)
                                {
                                    ...
                                    int vertexOffset = reader.ReadInt16(4);
                                    int occlusionID = reader.ReadInt16(6);
                                    GameObject obj = Instantiate(UIManager.instance.occlusionPrefab.gameObject, param1.transform);
                                    OcclusionCamera occlusion = obj.GetComponent<OcclusionCamera>();
                                    Vector3 pos = (Vector3)pbVar3.GetVertexBuffer().ReadSVector(vertexOffset) / GameManager.instance.translateFactor2;
                                    pos = new Vector3(pos.x, -pos.y, pos.z) * 2;
                                    occlusion.transform.localPosition = pos;
                                    occlusion.id = occlusionID;
                                    occlusionList.Add(occlusion);
                                    LensFlaresPreset presets = ScriptableObject.CreateInstance<LensFlaresPreset>();
                                    presets.presets = new LensFlares.FlarePreset[flareCount];

                                    do
                                    {
                                        iVar5 = reader.ReadUInt16(iVar4 - 2) & 0x3fff;
                                        LensFlares.FlarePreset preset = new LensFlares.FlarePreset();
                                        preset.sprite = timList[iVar5];
                                        preset.position = -((float)reader.ReadInt16(iVar4 - 4) / 0x1000 - 1f) + 1f;
                                        preset.startAngle = -90f;
                                        presets.presets[iVar3] = preset;
                                    ...
                                    LensFlares lensFlares = Instantiate(UIManager.instance.lensFlarePrefab, UIManager.instance.flaresRect);
                                    occlusion.lensFlare = lensFlares;
                                    occlusion.AwakeW();
                                    lensFlares._presets = presets;
                                    lensFlares._material = new Material(lensFlares._material);
                                    lensFlares._material.SetTexture("_OcclusionMap", occlusion._camera.targetTexture);
                                    lensFlares.color = new Color32(reader.ReadByte(0), reader.ReadByte(1), reader.ReadByte(2), 255);
                            ...
                            case 12:
                                ...
                                {
                                    iVar5 = 0x3ffe;
                                    puVar4 = LevelManager.instance.DAT_E58.mainTexture;
                                }
                                else
                                {
                                    if ((reader.ReadUInt16(16) & 0x3fff) == 0x3fff)
                                    {
                                        iVar5 = 0x3fff;
                                        puVar4 = LevelManager.instance.DAT_E48.mainTexture;
                                    }
                                    else
                                    {
                                        iVar5 = reader.ReadUInt16(16) & 0x3fff;
                                        puVar4 = timList[iVar5];
                                    }
                                }

                                iVar3 = Utilities.FUN_29BD0(puVar4.width);
                                writer.Write(18, (byte)(12 - (iVar3 - 1)));
                                sVar1 = (sbyte)puVar4.height;
                                writer.Write(19, (byte)(sVar1 - ((1 << (int)((uint)iVar3 - 1 & 31)) + 1)));
                                break;
                            case 13:
                                if (reader.ReadUInt16(22) == 0xffff)
                                    iVar5 = 0xffff;
        ...
        if (init)
        {
            ...
            if (pbVar3.mainT != rects)
            {
                pbVar3.mainT = rects;
                pbVar3.atlas = atlas;
                pbVar3.occlusions = occlusionList.ToArray();
            }
            ...
            for (int i = 0; i < materialIDs.Count; i++)
            {
                    ...
                    materialIDs[i] == 0x3fff)
                {
                    ...
                    pbVar3.materialIDs.Add(materialIDs[i], materialList.Count - 1);
                    pbVar3.subMeshCount++;
                    pbVar3.reflections = (Texture2D)levelManager.DAT_E48.mainTexture;
                }
                else if (materialIDs[i] == 0x3ffe)
                {
                    materialList.Add(levelManager.DAT_E58);
                    pbVar3.materialIDs.Add(materialIDs[i], materialList.Count - 1);
                    pbVar3.subMeshCount++;
                    pbVar3.glossiness = (Texture2D)levelManager.DAT_E58.mainTexture;
                }
                else
                {
                    pbVar3.materialIDs.Add(materialIDs[i], render[(int)renderList[materialIDs[i]]]);
                }
            }
