...
public class VigMesh : MonoBehaviour
{
    ...
    {
        new int[4095], new int[4095], new int[4095], new int[4095]
    };
    private int index;
    ...
    public BufferedBinaryReader GetVertexBuffer() { return brVar3; }
    ...
    public void CreateMeshData()
    {
        ...
        {
            MeshTopology _topology = j == linesSubmeshID ? topology : MeshTopology.Triangles;
    ...
    public void FUN_21F70(VigTransform param1)
    {
        ...
        /*if (GetComponent<Bobsled>() != null)
        {
            uVar9 = 0;
        }*/
        /*if (gameObject.name == "GlassWall")
        {
            uVar9 = 0;
        }*/
            ...
            switch (DAT_22FEC[(brVar13.ReadByte(3) & 0x3C) / 4 + ppuVar2])
            {
                ...
                case 0x80022330:
                    ...
                    if (true) //(Coprocessor.mathsAccumulator.mac0 > 0)
                    {
                        ...
                        int subMeshIndex = materialIDs[materialID];
                        Rect rect = mainT[materialID];
                        float width = Mathf.Clamp(atlas.width * rect.width - 1, 1, atlas.width);
                        float height = Mathf.Clamp(atlas.height * rect.height - 1, 1, atlas.height);
                        newUVs[index] = new Vector2
                        ...
                        newUVs[index + 2] = new Vector2
                            (Mathf.Clamp((float)brVar13.ReadByte(20) / width, min, max) * rect.width + rect.x,
                                (-1f + (rect.height - Mathf.Clamp((float)brVar13.ReadByte(21) / height, min, max) * rect.height)) + rect.y + 1f);
                        newTriangles[subMeshIndex][index2[subMeshIndex]] = index + 2;
                        newTriangles[subMeshIndex][index2[subMeshIndex] + 1] = index + 1;
                        newTriangles[subMeshIndex][index2[subMeshIndex] + 2] = index;
                        index += 3;
                        index2[subMeshIndex] += 3;
                        brVar13.Seek(32, SeekOrigin.Current);
                        iVar15 = 0x9000000;
                        goto LAB_22F08;
                    }
                ...
                case 0x80022C54:
                    ...
                    if (true) //(Coprocessor.mathsAccumulator.mac0 > 0)
                    {
                        int materialID = brVar13.ReadUInt16(16) & 0x3fff;
                        int subMeshIndex = materialIDs[materialID];
                        newVertices[index] = new Vector3
                            (Coprocessor.vector0.vx0, Coprocessor.vector0.vy0, Coprocessor.vector0.vz0) / tFactor;
                        newVertices[index + 1] = new Vector3
                        ...
                        int iVar20 = brVar13.ReadUInt16(18); //r10
                        int iVar21 = iVar20 << 16 >> 24; //r11
                        int iVar22 = Coprocessor.mathsAccumulator.mac2 >> iVar20; //r9
                        int iVar23 = Coprocessor.mathsAccumulator.mac1 >> iVar20; //r8
                        int iVar24 = iVar21 - iVar22; //r13

                        if (iVar24 < 0)
                            iVar22 = iVar21;

                        iVar23 += iVar22 << 8;
                        puVar5 = brVar13.ReadInt16(12);
                        Coprocessor.vector0.vx0 = brVar4.ReadInt16(puVar5);
                        Coprocessor.vector0.vy0 = brVar4.ReadInt16(puVar5 + 2);
                        Coprocessor.vector0.vz0 = brVar4.ReadInt16(puVar5 + 4);
                        iVar23 += (int)(0x101000U >> (iVar20 & 31));
                        Coprocessor.ExecuteMVMVA(_MVMVA_MULTIPLY_MATRIX.Rotation, _MVMVA_MULTIPLY_VECTOR.V0, _MVMVA_TRANSLATION_VECTOR.None, 12, false);
                        Vector2Int uv1 = new Vector2Int(iVar23 & 0xff, iVar23 >> 8 & 0xff);
                        iVar22 = Coprocessor.mathsAccumulator.mac2 >> iVar20; //r9
                        iVar23 = Coprocessor.mathsAccumulator.mac1 >> iVar20; //r8
                        iVar24 = iVar21 - iVar22;

                        if (iVar24 < 0)
                            iVar22 = iVar21;

                        iVar23 += iVar22 << 8;
                        puVar5 = brVar13.ReadInt16(14);
                        Coprocessor.vector0.vx0 = brVar4.ReadInt16(puVar5);
                        Coprocessor.vector0.vy0 = brVar4.ReadInt16(puVar5 + 2);
                        Coprocessor.vector0.vz0 = brVar4.ReadInt16(puVar5 + 4);
                        iVar23 += (int)(0x101000U >> (iVar20 & 31));
                        Coprocessor.ExecuteMVMVA(_MVMVA_MULTIPLY_MATRIX.Rotation, _MVMVA_MULTIPLY_VECTOR.V0, _MVMVA_TRANSLATION_VECTOR.None, 12, false);
                        Vector2Int uv2 = new Vector2Int(iVar23 & 0xff, iVar23 >> 8 & 0xff);
                        iVar22 = Coprocessor.mathsAccumulator.mac2 >> iVar20; //r9
                        iVar23 = Coprocessor.mathsAccumulator.mac1 >> iVar20; //r8
                        iVar24 = iVar21 - iVar22;
                        ...
                        iVar23 += iVar22 << 8;
                        iVar23 += (int)(0x101000U >> (iVar20 & 31));
                        Vector2Int uv3 = new Vector2Int(iVar23 & 0xff, iVar23 >> 8 & 0xff);
                        Color32 color = new Color32
                            (brVar13.ReadByte(0), brVar13.ReadByte(1), brVar13.ReadByte(2), 255 /*(byte)(brVar13.ReadByte(3) + 32)*/);
                        newColors[index] = color;
                        newColors[index + 1] = color;
                        ...
                        if (materialID == 0x3fff || materialID == 0xffff)
                        {
                            width = reflections.width - 1;
                            height = reflections.height - 1;
                        }
                        else if (materialID == 0x3ffe)
                        {
                            width = glossiness.width - 1;
                            height = glossiness.height - 1;
                        }

                        newUVs[index] = new Vector2((float)uv1.x / width, 1f - (float)uv1.y / height);
                        newUVs[index + 1] = new Vector2((float)uv2.x / width, 1f - (float)uv2.y / height);
                        newUVs[index + 2] = new Vector2((float)uv3.x / width, 1f - (float)uv3.y / height);
                        newTriangles[subMeshIndex][index2[subMeshIndex]] = index;
                        newTriangles[subMeshIndex][index2[subMeshIndex] + 1] = index + 1;
                        newTriangles[subMeshIndex][index2[subMeshIndex] + 2] = index + 2;
                        index += 3;
                        index2[subMeshIndex] += 3;
                        brVar13.Seek(24, SeekOrigin.Current);
                        iVar15 = 0x7000000;
